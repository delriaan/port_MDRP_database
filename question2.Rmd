...
mdrp_crosstab <- replicate(n = 30, { master_drug_data[(runif(n = length(alt_ndc)) <= 0.25)] %$% 
...
Even though the distribution of differences over the replications is bi-modal, the range is quite small, and most of the distribution is located at values greater than zero.  Therefore, I decided to use `dea_schedule` to split `master_drug_data` and `drug_category` as the "join-key" (see `?``event.vectors-package`` `).  Also, I limited the data to market dates starting in 2020 and forced all values in `termination_date` more than six months in the future (relative to "today") to six months in the future.

```{r PREP_DATA}
# Split 'master_drug_data' and store in attached environment 'nb_env' ("notebook environment")
master_drug_data[year(market_date) >= 2020] |>
  modify_at("termination_date", \(x) modify_if(x, \(i) (i - today()) > months(6), \(i) today() + months(6)))
...
mdrp_events$configure(
  ...
  , map.fields = {
      replicate(
        ...
        , expr = { c("drug_category", "market_date", "termination_date")}
        , simplify = FALSE
        )}
  , chatty = TRUE
  )
