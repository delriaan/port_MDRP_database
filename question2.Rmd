...
get.smart("drugs")$
  use(retain = NULL, subset = !cod_status %in% c(6:9, 11)) |>
  define(
    ...
    , ~rel_key + alt_ndc + fda_approval_date + market_date + termination_date + labeler_name 
        + dea_schedule + drug_category + pharm_class + fda_product_name
    ) |> 
  unique() |>
...
if (!"ev_plot" %in% ls()){
  ev_plot <- \(g){
    ...
    igraph::E(g)$title <- igraph::edge.attributes(g) %$% {
  	...
  	gph <<- g
  	visNetwork::visIgraph(igraph = g, physics = TRUE, type = "full") |>
  	visNetwork::visPhysics(solver = "forceAtlas2Based", timestep = 0.05) |>
  	visNetwork::visOptions(width = "1154", height = "1024") |>
    visNetwork::visEdges(shadow = TRUE) 
  }
}
gph <- mdrp_events$evt_graphs[sample(names(mdrp_events$evt_graphs), 1)] |> first();
gph_trace <- retrace.evs(event_graph = gph, evs = mdrp_events)
gph |> ev_plot() |> saveWidget(file = viz_file)
