...
> The use case for `event.vectors` is when you have more "join-key" values ($j$) expressed in orders of magnitude fewer event sources ($k: ||k|| \ll ||j||$)
...
Next, I created the event network, using `market_date` and `termination_date` to define event durations.  The significance of using this pair of temporal markers is that they are used to create a directed graph using _from_ &Rarr; _to_ sequencing.  Important parameter predicates used are as follows: 
...
list(
  ...
  # , c("mSt <= ?", "Only event pairs that begin no more than 60 days apart are retained")
  , c("time.control = [0, ?]", "Only event pairs that span no more than ? days are retained")
  ) |>
  imap(\(x, y){ 
    f <- if (y == 1){ tags$th } else { tags$td }
    tags$tr(
      f(class = "evs_params", x[1])
      , f(class = "evs_params", x[2])
      )
  }) |> 
  tags$table(style = "border-radius: 15px; border: solid 1px #DDDDDD; margin-bottom: 10px; padding:10px 15px; 10px; 20px; ") 
```

Predicates are resolved via Boolean *\&* having the global effect of reducing the number of edges across event graphs: 

:::{style="background-color:#CCCCCC; padding:10px; "}
Given the following:

- Fully connected graph $K_n := {n\choose2} \equiv \frac{n(n-1)}{2}$ <br><br>
- Predicate vector $\{V_k:v_1,v_2,v_3,\cdots,v\}_j: V_k|j \in \{0,1\}$ 

$K_n$ is reduced by <span style="font-weight:bold"> $1 - n^{-1}\sum{\Pi_{i=1}^n V_k|i}$ </span>, where each of $n$ nodes results from cross-temporal comparisons among $m$ event sources: $n = {m\choose2}$ :
:::
...
viz_file <- "./docs/viznetwork_%s.html"
...
ev_plot <- \(g){
  ...
  V(g)$title <- map(V(g)$trace, \(x){
	...
	V(g)$color <- degree(graph = g, mode = "in", loops = FALSE) |>
  ...
  E(g)$title <- edge.attributes(g) %$% {
	...
	visIgraph(igraph = g, physics = TRUE, type = "full") |>
  	...
  	visOptions(width = "1440", height = "1024", selectedBy = list(variable = "group", multiple = TRUE)) |> 
	  visEdges(smooth = FALSE)
}

#
mdrp_events$evt_graphs |> iwalk(\(x, y){ ev_plot(x) |> saveWidget(file = sprintf(viz_file, y)) });

# browseURL(sprintf(viz_file, "C0"))
```
