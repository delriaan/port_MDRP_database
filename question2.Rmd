...
Given event space $\mathbb{E}$ with $k$ independent events $$
\mathbb{E}:=\hat{e}_1 + \hat{e}_2+  \hat{e}_3 + \cdots + \hat{e}_k
$$
and corresponding heterogeneous data tables ($\mathbb{D} :=D_1,D_2,D_3,\cdots,D_k$) <br>and "join-key" ($J$, a column shared across $\mathbb{D}$), an event vector ($\overrightarrow{V_q}$) is formed having values indicating the existence or absence of selection over $D_k$ predicated on $q$ unique values $\forall J$: 
...
From each $\overrightarrow{V_q}$, $q$ subsets over $\mathbb{D}$ are derived as $\Gamma_q:=D_{k_q}|\overrightarrow{V_q}\ne\emptyset$. The number of records ($n$) can be represented as an array of row vectors: 

$$
\Gamma\Rightarrow\begin{array}
...
\begin{align}
K_\beta := {\beta\choose2} \equiv \frac{\beta(\beta-1)}{2}
\end{align}
$$
...
$$
\beta_q := \Pi_{i=1}^q\Gamma_i \Rightarrow (\beta_1,\beta_2,\beta_3,\cdots,\beta_q)\\
f(\beta_q) \mapsto \sum_{i=1}^q {\beta_i\choose2} \equiv\sum_{i=1}^q {\Pi(\Gamma_i)}
$$
which is also the number of comparisons ($N$) to be carried out computationally.

$N$ can be reduced by projecting a set of logically-reduced predicates ($p_m$) as mentioned earlier to interim results before creating each of the $q$ graphs:

$$
\beta_q^*:=\beta_q\times\Pi_{i=1}^m(p_m|G_q);\enspace p:0 \le p\le 1
$$ 
Depending on the values in $\mathbb{D}|p_m$, one may end up with $0 \le \beta^*_q\le\beta_q$ implying that all values for an interim result may be filtered out.  Trial and error may needed to fine-tune how graphs are generated as well as improve execution performance which is why the `graph.control` argument is provided for `event.vectors::make.evs_universe()` to allow for operating on graphs after the fact.  
