...
tags$table(
  style = "width:600px; "
  , tags$tr(
      tags$th(style = "text-align:middle; border: solid 2px #AAAAFF; background-color:#EEEEEE; "
              , "CRAN", colspan = 3, width="65%")
      , tags$th(style = "text-align:middle; border: solid 2px #AAAAFF; background-color:#AAAAAA; "
                , width = "*"
                , HTML("GitHub (use <code>remotes::install_github('delriaan/{package}', subdir = 'pkg')</code> to install)"))
      )
  , tags$tr(
      style = "align:top"
      , slider::slide(
...
The MDRP has many more NDC sequences due to truncation of leading zeroes. Fortunately, an NDC sequence is a collection of code segments (present in the data) concatenated with a hyphen. Knowing this, A function (`check_ndc_format()` &mdash; see `r tags$a(href = ".\\setup.R", target="_blank", "setup.R")` was created in order to derive conformed NDC segment sequences (using labeler and product codes) based on the OpenFDA sequences, allowing the MDRP and OpenFDA data to be joined later in the process.
...
.plot |>
  ...
  layout(
    title = list(
        text = HTML("Metrics Radar by Drug Route (Log<sub>30</sub> Days)<br><span style='font-size:smaller; '>(Double- or single-click legend items)")
...
## Question I<br><sup>Correlation</sup>

I decided to start asking some questions of the data given the metrics defined earlier.  The first question was, "How does the correlation between `days_to_market` and `on_market_age` change by drug route?" To proceed, I invoked custom package [`smart.data`](https://github.com/delriaan/smart.data):

```{r METRICS_CORRELATION} 
